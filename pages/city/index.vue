  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="button-container">
        <span>You can add city in here ></span>

        <NuxtLink :to="'createCity'">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(panels, index) in panel" :key="index">
              ...
              <td>{{ panels.delivery_price }} TMT</td>
              <td>
                <NuxtLink :to="`editCity/${panels.uuid}`">
      ...
      <!-- --------------------------------- -->
      <div class="table-concept" v-else></div>
      <!-- end  -->
    </div>
  </div>
</template>
    ...
    methods: {
      async getPanel() {
        const { data } = await axios.get(
          `http://95.85.125.197:1101/admin/cities/`,
          {
            headers: {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            const data = this.$axios.$delete(
              `http://95.85.125.197:1101/admin/cities/delete/${a}`,
              {
                headers: {
                  Authorization: "Bearer " + this.token
                }
