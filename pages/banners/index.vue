    ...
    <div class="card" v-else>
      ...
      <div class="button-container">
        ...
        <NuxtLink :to="'updateAccount'">
          <button class="primary" title="update account">
          ...
          <tbody>
            <tr v-for="(panels, index) in panel" :key="index">
              <td>
                <img
                  :src="
                    `http://95.85.125.197:1101/./public/banners/${panels.uuid}.webp`
                  "
                  alt=""
                />
              </td>
              <td>
                <NuxtLink :to="`bannerImage/${panels.uuid}`">
                  <b-button class="d-sm-down-none" variant="primary"
                    >Change banner</b-button
                  ></NuxtLink
                >
              </td>
              <td>
      ...
      <div class="table-concept" v-else>
        <input
          ...
          checked="checked"
        />
        <div class="table-display">Showing 1 to 20 of 95 items</div>
        <table>
          <thead>
            <tr>
              <th></th>
          ...
          <tbody>
            <tr v-for="(panels, index) in searchedInput" :key="index">
              <td>
              ...
              <td>
                <NuxtLink :to="{ name: 'productTable-editAdmin-id' }"
                  >Edit</NuxtLink
                >
              </td>
            </tr>
    ...
    methods: {
      async getPanel() {
        const { data } = await axios.get(
          `http://95.85.125.197:1101/admin/banners/`,
          {
            headers: {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            const data = this.$axios.$delete(
              `http://95.85.125.197:1101/admin/banners/delete/${a}`,
              {
                headers: {
                  Authorization: "Bearer " + this.token
                }
      ...
      async ban(a) {
        try {
          const data = await this.$axios.$post(
            "http://95.85.125.197:1101/admin/banners/add",
            this.image,
            {
              headers: {
                Authorization: "Bearer " + this.token
          ...
          Swal.fire({
            title: "Created new banner, please refresh the page"
          });
          this.panel = this.panel.filter(p => a == p.uuid);
