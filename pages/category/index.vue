    ...
    <div class="card" v-else>
      ...
      <div class="table-concept" v-if="inputSearch === ''">
        ...
        <table class="table table-hover" id="myTable">
          ...
          >
            <tr
              v-for="(panels, index) in categoryNew"
              :key="index"
              id="1"
              :value="categoryNew.indexOf(panels)"
            >
              <td class="index">
                <div>
                  {{ panels.name_tm }}
              ...
              <td>
                <img
                  :src="
                    `http://95.85.125.197:1101/./public/categories/${panels.uuid}.webp`
                  "
                  :alt="panels.name_ru"
                  class="img"
                />
              ...
              <td>
                <NuxtLink :to="`categoryEdit/${panels.uuid}`">
  ...
  export default {
    ...
    methods: {
      async categoryFunc() {
        ...
        for (let merjen of this.category) {
          merjen.subcategories.forEach(el => {
            this.subCat.push(el);
          });
        }

        for (let merjen of this.subCat) {
          merjen.subcategories.forEach(el => {
      ...
      async deleteProduct(a) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            this.categoryNew = this.categoryNew.filter(p => a !== p.uuid);
          }
        });
      },
      drop() {
      ...
      onChangeCategory() {
        const data = this.categoryNew.map((e, i) => ({
          id: e.uuid,
          order: i
        }));
        console.log("index");
